...
void callback(const nav_msgs::Odometry::ConstPtr& msg,const sensor_msgs::Image::ConstPtr& rgb_msg,const sensor_msgs::Image::ConstPtr& depth_msg)
{
   ...
   //別にここの位置に書く必要はない
    ...
    //feedback制御の動作ここから-------------------------------------------------------------------------------------------------------
      ...
      if(rotation_goal == false){
        if(Goal_RobotTH <= Est_RobotTH){
          rotation_goal = true;
        }
        robot_velocity.angular.z = 1000*(-Goal_RobotTH + Est_RobotTH);
      ...
      if(rotation_goal == true && ros::Time::now().toSec() - straight_begin.toSec()>4){
        ...
        robot_velocity.linear.x = 1000*(sqrt(pow(Goal_RobotX-Est_RobotX,2)+pow(Goal_RobotY-Est_RobotY,2)));
  ...
  std::cout<<"-----------------------------------"<<img_depth.at<float>(239,320)<<"-----------------------------------------"<<std::endl;
...
int main(int argc,char **argv){
    ...
    if(kaisu!=0){ros_begin = straight_begin = ros::Time::now();}
  ...
  //LX=3.0,VX=0.25,omegaZ=2.15,THZ=0.25;
    ...
    MarkerW[1]= (cv::Mat_<float>(3, 1) <<1.0, 0.28, 6.0);//(左、高さ、前)
